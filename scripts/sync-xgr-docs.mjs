...
async function getJson(url) {
  const response = await fetch(url, { headers: { accept: 'application/vnd.github+json', 'user-agent': 'xgr-mcp-gateway-build' } });
  if (!response.ok) throw new Error(`[sync-xgr-docs] HTTP ${response.status} for ${url}`);
  return response.json();
}

async function getText(url) {
  const response = await fetch(url, { headers: { accept: 'application/vnd.github.raw', 'user-agent': 'xgr-mcp-gateway-build' } });
  if (!response.ok) throw new Error(`[sync-xgr-docs] HTTP ${response.status} for ${url}`);
